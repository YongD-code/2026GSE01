...
enum class Facing { South, SouthWest, West, NorthWest, North, NorthEast, East, SouthEast };
struct SpriteLayout {
...
class SpriteSheet {
    struct Frame {float u0,v0,u1,v1;int width,height;float anchorX;};
    ...
    static GLuint Compile(GLenum kind,const char* source) {
        ...
        if(!ok) {char log[2048]={};glGetShaderInfoLog(shader,sizeof(log),nullptr,log);std::cerr<<log<<std::endl;glDeleteShader(shader);return 0;}
        return shader;
    }
public:
    SpriteSheet()=default;
    ...
    ~SpriteSheet() {if(texture)glDeleteTextures(1,&texture);if(program)glDeleteProgram(program);if(vao)glDeleteVertexArrays(1,&vao);}
    bool Ready() const {return texture&&program&&vao;}
    bool Load(const std::wstring& relativePath,const SpriteLayout& definition=SpriteLayout()) {
        using Microsoft::WRL::ComPtr;
        layout=definition;
        if(layout.columns<=0||layout.rows<=0||layout.walkFrames<=0||layout.firstWalkFrame<0||
           layout.firstWalkFrame+layout.walkFrames>layout.columns||layout.idleFrame<0||layout.idleFrame>=layout.columns) return false;
        for(int row:layout.directionRows) if(row<0||row>=layout.rows) return false;
        HRESULT initialized=CoInitializeEx(nullptr,COINIT_APARTMENTTHREADED);
        struct ComScope {bool owned;~ComScope(){if(owned)CoUninitialize();}} comScope={SUCCEEDED(initialized)};
        ...
        std::wstring folder=executable;size_t slash=folder.find_last_of(L"\\/");
        folder=slash==std::wstring::npos?L"":folder.substr(0,slash+1);
        const std::wstring candidates[]={folder+relativePath,relativePath,L"SimpleGame/"+relativePath};
        ComPtr<IWICBitmapDecoder> decoder;
        for(const auto& path:candidates) {
            if(SUCCEEDED(factory->CreateDecoderFromFilename(path.c_str(),nullptr,GENERIC_READ,WICDecodeMetadataCacheOnLoad,decoder.ReleaseAndGetAddressOf()))) break;
        }
        if(!decoder) return false;
        ComPtr<IWICBitmapFrameDecode> source;
        if(FAILED(decoder->GetFrame(0,source.GetAddressOf()))) return false;
        ...
        if(width<static_cast<UINT>(layout.columns)||height<static_cast<UINT>(layout.rows)||width>8192||height>8192) return false;
        ...
        for(int row=0;row<layout.rows;++row) for(int col=0;col<layout.columns;++col) {
            ...
            if(maxX<minX) {minX=left;maxX=left;minY=top;maxY=top;}
            int w=maxX-minX+1,h=maxY-minY+1;
            float anchor=w*.5f;
            if(layout.anchorAtFeet) {
                double sum=0,weight=0;
                for(int y=maxY-h/8;y<=maxY;++y) for(int x=minX;x<=maxX;++x) {
                    unsigned char alpha=pixels[(static_cast<size_t>(y)*width+x)*4+3];
                    if(alpha>24) {sum+=(x-minX)*double(alpha);weight+=alpha;}
                }
...
void main(){vec4 pixel=texture(image,uv);if(pixel.a<.01)discard;
    ...
    result=vec4(linearScene?linearColor*(1.0+emission*bright):color,pixel.a*tint.a);
        ...
        glGenVertexArrays(1,&vao);return Ready();
    }
    void Draw(Renderer& renderer,Point feet,float size,const SpriteAnimation& animation,Color tint,int width,int height,float emission=0.f) {
        ...
        glUniform4f(glGetUniformLocation(program,"rectangle"),feet.x-frame.anchorX*scale,feet.y-h,w,h);
        glUniform4f(glGetUniformLocation(program,"uvRect"),frame.u0,frame.v0,frame.u1,frame.v1);
        ...
        glDrawArrays(GL_TRIANGLES,0,6);glBindTexture(GL_TEXTURE_2D,0);glBindVertexArray(0);glUseProgram(0);
    }
};
